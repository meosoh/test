...
def make_sample_tables() -> List[Dict[str, Any]]:
    return [
        {
            "id": 1,
            "table_name": "SUS304 切削部品標準単価表",
            "item_category": "切削部品",
            "material": "SUS304",
            "process": "旋削",
            "supplier": "A社",
            ...
            "version": 1,
            "status": "承認済",
            "description": "小径シャフト向けの基準単価テーブル",
            "updated_by": "Ryo",
            "updated_at": "2026-04-14 09:00",
            "details": [
                {
                    "condition": "外径 0-50 mm",
                    "unit_price": 1200,
                    "correction_factor": 1.00,
                    "note": "標準",
                },
                {
                    "condition": "外径 50-100 mm",
                    "unit_price": 1800,
        ...
        },
        {
            ...
            "version": 2,
            "status": "下書き",
            "description": "薄板部品向け。再見積中。",
            "updated_by": "Ryo",
            "updated_at": "2026-04-14 10:30",
            "details": [
                {
                    "condition": "板厚 0-3 mm",
                    "unit_price": 900,
            ...
            "history": [
                {
                    "changed_at": "2026-04-14 10:30",
                    "changed_by": "Ryo",
                    "change_summary": "補正係数を見直し中",
                }
            ],
        },
    ]
...
def initialize_state() -> None:
    if "cost_tables" not in st.session_state:
        st.session_state.cost_tables = make_sample_tables()
    if "selected_table_id" not in st.session_state:
        st.session_state.selected_table_id = st.session_state.cost_tables[0]["id"]
...
def get_tables() -> List[Dict[str, Any]]:
    return st.session_state.cost_tables
...
def get_next_id() -> int:
    ...
    if not tables:
        return 1
    return max(t["id"] for t in tables) + 1


def add_history_entry(table: Dict[str, Any], summary: str, user: str = "Ryo") -> None:
    table.setdefault("history", []).insert(
        0,
        {
...
def validate_table_payload(payload: Dict[str, Any], details_df: pd.DataFrame) -> List[str]:
    errors: List[str] = []
...
def serialize_details_df(details_df: pd.DataFrame) -> List[Dict[str, Any]]:
    cleaned = details_df.fillna("").to_dict(orient="records")
    results = []
    for row in cleaned:
        results.append(
            {
                ...
                "correction_factor": float(row.get("correction_factor", 1) or 1),
...
def create_or_update_table(table_id: int | None, payload: Dict[str, Any], details_df: pd.DataFrame) -> int:
    tables = get_tables()
    details = serialize_details_df(details_df)
...
def duplicate_table(table_id: int) -> int:
    source = get_table_by_id(table_id)
    if source is None:
        raise ValueError("複製元が見つかりません。")
...
def summarize_evidence(table: Dict[str, Any]) -> Dict[str, float]:
